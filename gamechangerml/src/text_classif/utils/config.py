...
def read_verify_config(config_file):
    try:
        ...
        logger.info(f"verifying config : {cfg_name}")
...
def _check_cfg(cfg_dict):
    ...
    if diff := req.symmetric_difference(cfg_in):
        msg = f"missing or unknown config items: {diff}"
        raise ValueError(msg)

    for k, v in cfg_dict.items():
        if k not in cfg_schema:
            bail = True
            logger.error(f"missing/unknown config item : {k}")
        else:
            type_ = type(cfg_dict[k])
            req = cfg_schema[k]
            if isinstance(cfg_schema[k], tuple):
                if type_ not in req:
                    logger.error(f"{k}: expecting type in {req}; got {type_}")
                    bail = True
            elif type_ != req:
                logger.error(f"{k}: expecting type {req}; got {type_}")
                bail = True
            if v is None:
                nones[k] = None
    if bail:
    ...
    if cfg_dict["max_seq_len"] is not None:
        ...
        logger.info(f'max seq len : {cfg_dict["max_seq_len"]}')
        logger.info(f'   truncate : {cfg_dict["truncate"]}')
...
def _is_verified(cfg_data):
    ...
    if cfg_data["num_labels"] < 2:
        verified = False
        logger.error(f'must have at least 2 labels, got {cfg_data["num_labels"]}')
    if (
        cfg_data["clip_grad_norm"] is not None
        and cfg_data["clip_grad_norm"] <= 0.0
    ):
        verified = False
        logger.error("invalid `clip_grad_norm")
    if cfg_data["tensorboard_path"] is not None and not os.path.isdir(
        cfg_data["tensorboard_path"]
    ):
        verified = False
    ...
    if cfg_data["model_name"] not in supported_types:
        verified = False
        logger.error(f'model type not supported : {cfg_data["model_name"]}')
